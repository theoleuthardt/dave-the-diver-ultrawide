...
public class Plugin : BasePlugin
{
    ...
    public override void Load()
    {
        ...
        EnableIndicatorCameraFix = Config.Bind(
            ...
            "Every frame, forces InputActionIndicatorPanel.m_Camera back to a full rect and calls Camera.ResetAspect() on it, right before the game's own LateUpdate computes the prompt's screen position via WorldToViewportPoint. CONFIRMED RUNNING CORRECTLY ON REAL HARDWARE BUT INEFFECTIVE: the camera's rect/aspect/identity were all already correct at every logged sample, and the prompt was still reported mispositioned — so this was not actually the bug. Left on (confirmed harmless) but the real cause is still unidentified; see docs/research-notes.md.");
        EnableCameraManagerCrossCheck = Config.Bind(
            "Patches", "EnableCameraManagerCrossCheck", true,
            "Sub-part of EnableIndicatorCameraFix (has no effect if that's off): also checks whether EvilFactory.CameraManager.mainCamera is a *different* Camera object than the one InputActionIndicatorPanel uses, and fixes that one too if so. Previously lived in CameraResolution.UpdateCanvasScale and CONFIRMED TO CRASH THE GAME there (ran too early, before any dive scene/CameraManager existed) — moved here, where it only ever runs once an interact prompt is actually visible, i.e. guaranteed to be in a real gameplay scene. Confirmed safe on real hardware (no crash), but the divergence it checks for never fired once in a full dive session — either it's never actually the case, or EvilFactory.CameraManager never had a live instance to compare against. Not confirmed to fix anything. See docs/research-notes.md.");
